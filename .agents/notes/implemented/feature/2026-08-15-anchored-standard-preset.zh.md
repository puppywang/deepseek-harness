...
以 opt-in 方式随附 `anchored-standard` agent preset，位于 `apps/cli/config/agent-presets/anchored-standard/`，`order: 5`；`default` 仍为 `standard`。它是在 `standard` 组装上叠加两处差异：
...
- **现在就把 `anchored-standard` 设为默认 preset**——否决：支撑证据只有两轮基准运行；当首轮没有工具调用时，晋升触发条件会把 agent 留在两个工具上。默认化应等待更强证据与剩余限制得到更强证据。
...
- roster 在既有四个 preset 之后新增一个实验条目；`default: standard` 不变，已有会话不受影响。
