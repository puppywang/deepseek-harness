...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - `apps/desktop` is a private Electron shell: in development it starts the built `dsh web` application with the system Node executable, while packaged builds use the bundled Node runtime and production `dsh` deployment on loopback; it loads the existing Web client, and Electron's main process owns the auto-hidden native application menu, the desktop-only directory picker, path opener, external-link handoff, download save dialog, and GitHub-Release update lifecycle through token-authenticated loopback requests from the Host; an IPC fetch carrier remains a future transport extension.
...
| Carrier layer | `dsh-host-webserver` | HTTP and upgrade: static serving + `/api/*`→handler forwarding + WebSocket upgrade route + close semantics; plugin bundle endpoint + `__DSH_BOOT__` manifest injection (fed by the web plugin registry) | Browser and the current Electron loopback shell reuse it; zero workspace dependencies (the registry arrives by structural injection); a future IPC carrier can avoid it |
...
1. **Pick a fetch impersonation**: browser or Electron loopback HTTP/WebSocket / in-process `host.handler.fetch` injection / your own transport-aspect subclass (e.g. a future Electron IPC carrier, see the "Subclass table" below).
...
The Web and current Electron applications mount Host, the Web carrier, and the browser client composition; `dsh --profile headless` mounts a direct core runner with zero Host, HTTP, or ports. ACP-class protocol bridges do not follow the client-carrier checklist: they expose core to the external ecosystem and mount directly via `ctx.plugin(entry-point plugin)` without fetch.
...
| Electron desktop shell | `apps/desktop` | existing `WebApiClient` over loopback HTTP and WebSocket | starts `dsh web` with the development system Node or packaged Node/runtime deployment and reuses the Web carrier; the shell owns the window, the auto-hidden native application menu, child-process lifecycle, desktop-only native interactions, external-link handoff, download save dialog, and GitHub-Release update lifecycle |
| IPC bridge subclass | an Electron shell | IPC serialization round trip | future transport option; would swap only `doFetch` and keep the contract and base class unchanged |
