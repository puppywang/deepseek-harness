...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - `apps/desktop` is a private Electron shell: in development it starts the built `dsh web` application with the system Node executable, while packaged builds use the bundled Node runtime and production `dsh` deployment on loopback; it loads the existing Web client, and Electron's main process owns the desktop-only directory picker, path opener, external-link handoff, download save dialog, and GitHub-Release update lifecycle through token-authenticated loopback requests from the Host; an IPC fetch carrier remains a future transport extension.
...
| Electron desktop shell | `apps/desktop` | existing `WebApiClient` over loopback HTTP and WebSocket | starts `dsh web` with the development system Node or packaged Node/runtime deployment and reuses the Web carrier; the shell owns the window, child-process lifecycle, desktop-only native interactions, external-link handoff, download save dialog, and GitHub-Release update lifecycle |
