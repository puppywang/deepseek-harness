...
Host Remote service for profile plugin management. `PluginManagerGateway` registers the `pluginManager` service and publishes Typert-generated Remotes for `list`, `catalog`, `install`, `update`, `uninstall`, and `restart`. It composes the shared `@deepseek-ai/dsh-plugin-manager` library with the subprocess seam, so pnpm executes through the bundled `@pnpm/exe` binary without requiring a system pnpm — the path that makes plugin installation available inside the packaged desktop app. `catalog` searches npm for the top 250 packages carrying the `dsh-plugin` keyword, then verifies each latest manifest declares a DSH bundle or client role.
...
- **Catalog is a bounded popular slice** — npm reports more keyword matches than the 250 candidates verified here; full paging can be added later. npm availability and rate limits apply; keyword-only packages without a DSH bundle/client manifest are skipped. Verified results are cached briefly.