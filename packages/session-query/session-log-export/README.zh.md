...
| `/export <path>` | 错误；Web 下载通过浏览器的普通下载行为选择目标位置，Electron 桌面壳显示原生保存对话框 |
...
两条入口都会对 `GET /api/session.export?...` 发出 `HEAD` 预检，然后把 GET URL 交给浏览器下载管理器，JavaScript 不缓冲 ZIP。一个控制器按会话持有一项进行中的下载，把并发操作折叠进该任务，并在插件释放时取消预检。弹窗状态存放在按会话键控的快照存储中，因此按钮与命令按会话共享一个弹窗。Electron 主进程会接收产生的下载并显示原生保存对话框，不改变浏览器控制器。
...
- **浏览器下载，而非 Host 路径写入**——Web 由浏览器下载管理器选择目标位置，Electron 由原生保存对话框选择；不会返回 Host 路径或原生文件夹操作。
