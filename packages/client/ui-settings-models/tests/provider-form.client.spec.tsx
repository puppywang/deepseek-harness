...
describe('protocolChoices', () => {
  ...
  it('reads the thinking levels out of the model schema and nothing else', () => {
    const { namespace } = scriptedFace()
    expect(reasoningEffortLevels(namespace, settingsSchema)).toEqual(REASONING_LEVELS)
    expect(reasoningEffortLevels(undefined, settingsSchema)).toEqual([])
    const plain = { ...namespace, schema: JSON.parse(JSON.stringify(Schema.object({}).toJSON())) as unknown }
    expect(reasoningEffortLevels(plain, settingsSchema)).toEqual([])
...
describe('endpoint interrogation', () => {
  ...
  it('keeps the create card asking only once it has an endpoint', () => {
    ...
    render(
      <CustomProviderCard
        taken={[]} protocols={PROTOCOLS} reasoningLevels={REASONING_LEVELS} revision={7} operations={operationsWith(scripted.face)}
