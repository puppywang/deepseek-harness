...
Browser **Plugins** tab that manages profile plugins from Web Settings. The client registers one `settings.plugins.tab` contribution with id `manage` (after the read-only `all` inventory tab). It lazily reads `ctx.remote.pluginManager.list()` and offers a GitHub `dsh-plugin` discovery list (`pluginManager.catalog`) plus install (spec + enable row), update, and uninstall actions through the same Remote. The Host re-applies the patch stack to the running Loader, so changes apply live without restart; when the Host reports `restartRequired: true`, the tab opens a confirmation dialog warning that running conversations will be interrupted before calling `pluginManager.restart`.
...
- **Catalog is GitHub-topic based** — the tab lists repositories tagged `dsh-plugin`; curated/npm registry metadata is not shown yet.
