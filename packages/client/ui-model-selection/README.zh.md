...
- **推理强度记忆仅存于浏览器本地**——每个提供方／模型路由最后一次显式选择的推理强度存在 localStorage 中，不会跨设备或浏览器 profile 同步。
