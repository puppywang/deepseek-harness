...
每个打包命令都会在 Electron Builder 之前审计运行时的静态及配置导入，并对内置的 `dsh web` 执行冒烟测试；Electron Builder 的 `afterPack` 还会再次检查安装包内的启动包清单。生产运行时会移除 source map、调试符号、异平台可选 native 包和未使用的原生 prebuild，同时保留目标平台所需的原生模块。桌面壳不会把 `node_modules/**` 作为 Electron 的全量 `asarUnpack` 规则；外置运行时是因为独立 Node 子进程不能直接读取 `app.asar`。
